 ...
 *
 */
public class CompletionPostingsFormat extends PostingsFormat {

    public static final String CODEC_NAME = "completion";
    public static final int SUGGEST_CODEC_VERSION = 1;
    public static final String EXTENSION = "cmp";

    private PostingsFormat delegatePostingsFormat;
    private final static Map<String, SuggestLookupProvider> providers;
    private SuggestLookupProvider writeProvider;
    
    static {
        final SuggestLookupProvider provider = new AnalyzingSuggestLookupProvider(true, false, true, false);
        final Builder<String, SuggestLookupProvider> builder = ImmutableMap.builder();
        providers = builder.put(provider.getName(), provider).build();
    }

    public CompletionPostingsFormat(PostingsFormat delegatePostingsFormat, SuggestLookupProvider provider) {
        super(CODEC_NAME);
    ...
    private class SuggestFieldsProducer extends FieldsProducer {
        ...
        public SuggestFieldsProducer(SegmentReadState state) throws IOException {
            ...
            try {
                ...
                SuggestLookupProvider suggestLookupProvider = providers.get(providerName);
                if (suggestLookupProvider == null) {
                ...
                if (state.context.context != Context.MERGE) {
                    this.lookupFactory = suggestLookupProvider.load(input);
                }
                success = true;
            } finally {
                if (!success) {
